...
namespace CourseraAssignments.AdvancedAlgorithms.Week2
{
    class NaiveSimplex
    {
        ...
        //    long[] LastRow = Console.ReadLine().Split().Select(long.Parse).ToArray();
        ...
        //    long[] LastLastRow = Console.ReadLine().Split().Select(long.Parse).ToArray();
        ...
        //    string Result = Solve((int) N, (int) M, matrix);
        ...
        public static string Solve(int N, int M, double[,] matrix1)
        {
            ...
            int jj = 0;
            foreach (double result in BestAnswer)
            {
                double res = Math.Abs(result);
                if (jj == BestAnswer.Length - 1) break;
                if (res != (int)res)
                {
                    Result += (("{0:N3}"), (double)res) + " ";
                }
                else
                {
                    Result += res + ".000 ";
        ...
        private static bool CheckAllConstraints(long N, long M, double[] vertex, double[,] matrix1)
        {
            for (long i = 0; i < N; i++)
            {
                ...
                if (RoundDouble(LeftSide) > matrix1[i, M])
                {
                    return false;
                }
            }
